...
func (a *DiscoveryActor) Receive(ctx actor.Context) {
	...
	case *actor.Started:
		if err := pubsub.Subscribe(route.TOPIC_REPLY, ctx.Self(), Parse); err != nil {
			logs.LogError.Printf("subscribe pubsub to %s error: %s", route.TOPIC_REPLY, err)
			break
		}
